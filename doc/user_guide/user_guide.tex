...
Each time a decompilation is triggered, plugin checks that it is properly configured. If it is not, warning is displayed, then the configuration dialog appears. Here, you have to make sure plugin finds the \texttt{retdec-decompiler.sh} script. Either set path to \texttt{retdec-decompiler.sh} in the configuration dialog, or add the decompiler's \texttt{bin} directory to the system \texttt{PATH}.
